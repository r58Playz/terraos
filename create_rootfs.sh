...
if ! which pacstrap >/dev/null 2>/dev/null; then
  die "this program requires pacstrap"
fi

if ! which arch-chroot >/dev/null 2>/dev/null; then
  die "this program requires arch-chroot"
fi
...
arch-chroot "${1}" bash -c 'pacman --noconfirm -U *.zst' || die "failed to install packages"
arch-chroot "${1}" bash -c 'rm *.zst' || die "failed to remove packages"
...
umount mnt || die "failed to unmount shim"
...
umount mnt || die "failed to unmount recovery image"
